...
library(ggthemes)
###The percentage of protected area was calculated as the number of grid cells within protected areas divided by the total number of grid cells and multiplied by 100. All spatial datasets were initially harmonized to a common geographic coordinate reference system (EPSG:4326; Supplementary Fig. S15)
data <- read.table("2.Stacked Bar Chart for protected areas_EPSG4326.zip/protected_areas_traits_Fig.S15B.txt", header = T, sep = '\t', check.names = FALSE)
...
p1
saveRDS(p1, paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/traits_enzyme.rds"))
ggsave(plot = p1, filename = paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/traits_enzyme.png"),width = 5, height = 7, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_EPSG4326.zip/protected_areas_gene_richness_Fig.S15A_1.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p2<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p2
saveRDS(p2, paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_richness_Fig.S15A_1.rds"))
ggsave(plot = p2, filename = paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_richness_Fig.S15A_1.png"),width = 4, height = 6.4, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_EPSG4326.zip/protected_areas_gene_dissimilarity_Fig.S15A_2.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p3<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p3
saveRDS(p3, paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_dissimilarity_Fig.S15A_2.rds"))
ggsave(plot = p3, filename = paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_dissimilarity_Fig.S15A_2.png"),width = 4, height = 6.4, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_EPSG4326.zip/protected_areas_gene_richness_dissimilarity_Fig.S15A_3.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p4<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p4
saveRDS(p4, paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_richness_dissimilarity_Fig.S15A_3.rds"))
ggsave(plot = p4, filename = paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/protected_areas_gene_richness_dissimilarity_Fig.S15A_3.png"),width = 4, height = 6.4, dpi = "retina")

###To avoid potential latitudinal distortions associated with variations in grid-cell area, all percentage calculations were subsequently repeated using the equal-area Eckert IV projection.
data <- read.table("2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection.zip/protected_areas_traits.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p1
saveRDS(p1, paste0("/2.Stacked Bar Chart for protected areas_EPSG4326", "/traits_enzyme.rds"))
ggsave(plot = p1, filename = paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/traits_enzyme.png"),width = 5, height = 7, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection.zip/protected_areas_gene_richness_Fig1_1.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p2<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p2
saveRDS(p2, paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/protected_areas_gene_richness_Fig1_1.rds"))
ggsave(plot = p2, filename = paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/protected_areas_gene_richness_Fig1_1.png"),width = 4, height = 6.4, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection.zip/protected_areas_gene_dissimilarity_Fig1_2.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p3<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p3
saveRDS(p3, paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/protected_areas_gene_dissimilarity_Fig1_2.rds"))
ggsave(plot = p3, filename = paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/protected_areas_gene_dissimilarity_Fig1_2.png"),width = 4, height = 6.4, dpi = "retina")

data <- read.table("2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection.zip/protected_areas_gene_richness_dissimilarity_Fig1_3.txt", header = T, sep = '\t', check.names = FALSE)
DJ <- melt(data,id.vars='x_axis')
names(DJ)[1:2] <- c("Taxonomy","sample")  
DJ$sample <- factor(DJ$sample, levels = unique(DJ$sample))       
DJ$Taxonomy <- factor(DJ$Taxonomy, levels = unique(DJ$Taxonomy)) 
...
p4<-ggplot(DJ, aes( x = sample,y = value, fill = Taxonomy))+  
  ...
  scale_fill_manual(values=c("#FFFEE0","#C0C000"))  
p4
saveRDS(p4, paste0("/2.Stacked Bar Chart for protected area_equal-area Eckert IV projection", "/protected_areas_gene_richness_dissimilarity_Fig1_3.rds"))
ggsave(plot = p4, filename = paste0("/2.Stacked Bar Chart for protected areas_equal-area Eckert IV projection", "/protected_areas_gene_richness_dissimilarity_Fig1_3.png"),width = 4, height = 6.4, dpi = "retina")
...
varechem=varechem[c(2:4,6:11,13:15,17)]
env<-as.matrix(varechem)
co_linear <- varclus(env, similarity="spear") # spearman is the default
co_linear
p2=plot(co_linear)
